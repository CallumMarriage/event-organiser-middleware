...
export function createEventsTable(next) {
  ...
  export function getEventsFromSet(requestId, user_id, callback){
    const pool = require('../utils/postgres.js');
    console.log('>> ' + requestId   + ", " + user_id);
    const query = escape('SELECT * FROM events WHERE event_id = ANY(SELECT event_id FROM  users_to_events WHERE user_id=$1)');
